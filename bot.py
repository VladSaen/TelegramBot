...
# Допоміжна функція для перевірки, чи є користувач електриком
def is_admin(user_id):
    """Перевіряє, чи є наданий ID електриком."""
    try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Привіт! ⚡ Я ваш асистент для зв'язку з електриком. "
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Надсилає довідкову інформацію, різну для електрика та користувача."""
    user_id = update.effective_user.id
    if is_admin(user_id):
        await update.message.reply_text(
            "👨‍💼 **Інструкції для електрика:**\n\n"
...
async def admin_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Перевіряє статус конфігурації бота (тільки для електрика)."""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Ця команда доступна лише електрику.")
        return
        
    status_text = (
        f"🤖 **Статус Бота (Електрик):**\n"
        f"-----------------------------------\n"
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'action_text_guide':
        await query.edit_message_text(
            text="✍️ **Режим текстової заявки**\n\nПросто напишіть своє повідомлення або питання. Я відразу передам його електрику.",
            parse_mode='Markdown'
        )
        ...
        help_text = (
            ...
            "Цей бот слугує для анонімної передачі заявок та питань електрику. "
            "Електрик отримує ваше повідомлення та може відповісти вам через бота.\n\n"
            "**Важливо:** Ваш Telegram ID передається електрику для забезпечення можливості відповіді."
        )
        await query.edit_message_text(text=help_text, parse_mode='Markdown')


# Команда /reply для відповіді електрика
async def handle_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Дозволяє електрику відповісти користувачу за його ID."""
    
    # 1. Перевірка, чи це електрик
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Ця команда доступна лише електрику.")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обробляє вхідні повідомлення і пересилає їх електрику."""
    ...
    # Якщо це повідомлення від самого електрика, ігноруємо його, якщо це не /reply
    if is_admin(user.id):
        return
    ...
    try:
        ...
        await message.reply_text("❌ Помилка конфігурації бота. Електрик не знайдений.")
        return
    ...
    # Створення шаблону повідомлення для електрика (надсилається окремим текстом)
    admin_notification = (
        f"📩 **НОВА ЗАЯВКА**\n"
    ...
    if is_success:
        await message.reply_text("✅ Дякую! Вашу заявку успішно передано електрику ⚡")
    else:
